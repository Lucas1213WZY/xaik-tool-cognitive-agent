...
- ❌ `src/cr_agent/agents/` (headless_policies.py, counterfactual_strategies.py, forward_meta_router.py)
- ❌ `src/cr_agent/environments/` (meta_router_env.py, counterfactual_meta_router.py)
...
- Main functions:
  - `load_forward_strategies()`: Dict[str, strategy] from reasoning_strategies API
  - `run_meta_on_batch()`: Execute forward episode with meta model strategy selection
- Imports directly from `src.reasoning_strategies`
...
- Main function: `load_counterfactual_strategies()`: Dict[str, strategy] from reasoning_strategies API
- Imports directly from `src.reasoning_strategies`
...
All strategies now loaded from `src.reasoning_strategies` API:
...
2. **Single Source of Truth**: Strategies loaded from reasoning_strategies API, not reimplemented
...
```python
from src.cr_agent import load_forward_strategies, run_meta_on_batch
...
- ❌ `from src.cr_agent.agents import ...` → ✅ `from src.cr_agent import ...`
- ❌ `from src.cr_agent.environments import ...` → ✅ `from src.cr_agent import ...`
- ❌ `from src.cr_agent.headless_policies import ...` → ✅ `from src.cr_agent import ...`
