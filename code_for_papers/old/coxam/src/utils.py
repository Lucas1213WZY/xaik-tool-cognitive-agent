...
def generate_tree_structure(max_depth, num_classes=2, num_features=5):
    nodes = []
    node_counter = 0
    ...
    def build(current_depth):
        ...
        if current_depth == max_depth:
            # Leaf node with random class probabilities
            values = np.random.dirichlet(np.ones(num_classes)).tolist()
            nodes.append({"node": my_id, "is_leaf": True, "value": values})
            return my_id
        else:
            # Internal node
            feature_idx = np.random.randint(0, num_features)
            base_feature = f"a{feature_idx}"
            # if np.random.random() < 0.3:  # 30% chance for categorical feature
            #     cat_idx = np.random.randint(0, 5)  # Arbitrary category index up to 4
            #     feature = f"{base_feature}={cat_idx}"
            #     threshold = 0.5  # Typical for binary one-hot
            # else:
            feature = base_feature
            threshold = np.random.uniform(0, 1)  # Arbitrary range for continuous

            left_id = build(current_depth + 1)
            right_id = build(current_depth + 1)
            nodes.append({
                "node": my_id,
                "feature": feature,
                "threshold": threshold,
...
def create_random_decision_tree_interpreter(max_depth, num_classes=2, num_features=5, app_id='random', model_name='random_model'):
    tree_structure = generate_tree_structure(max_depth, num_classes, num_features)
    fidelity = np.random.uniform(0.9, 1.0)
    class_labels = [f"class_{i}" for i in range(num_classes)]

    # Create explanation_df
    explanation_data = {
        'appId': [app_id],
        'fidelity': [fidelity],
    ...
    # Create metadata_df with feature names and some category labels
    metadata_data = {'appId': [app_id]}
    for i in range(num_features):
        metadata_data[f'a{i}'] = [f'feature_{i}']
        # for j in range(5):  # Assume up to 5 categories per feature
        #     metadata_data[f'v{i}_{j}'] = [f'cat_{j}']
    metadata_df = pd.DataFrame(metadata_data)

    # Instantiate the interpreter
    interpreter = DecisionTreeInterpreter(explanation_df, metadata_df, app_id, model_name)
    return interpreter
...
# # ===== cog_timing.py (or put near your helpers) =====
# import math
# import numpy as np

# # ---- Shared constants (align with your LR constants) ----
# EPS            = 0.05
# SQRT2          = math.sqrt(2.0)

# # UI/Memory reading latencies (kept tiny & symmetric across LR/DT)
# T_READ_NUM     = 2.0    # read a number on-screen
# T_READ_TEXT    = 2.0    # read a label / feature name on-screen
# T_EQ_CHECK     = 2.0    # equality check for categoricals (matches your LR default)
# T_DECISION     = 2.0    # fixed motor/choice overhead at the end

# def _phi(z):
#     return 0.5 * (1.0 + math.erf(z / SQRT2))

# def ans_compare_time_and_prob(val: float, thr: float, w_eff: float, T_COMP0: float, T_COMP1: float):
#     """
#     Comparator with both noise (for probability) and time (for latency).
#     Var ~ (w_eff*|val|)^2 + (w_eff*|thr|)^2  -> sd
#     p_left = P(val <= thr) = Φ((thr - val)/sd)
#     t_cmp ≈ T_COMP0 + T_COMP1 / max(d', EPS), where d' = |val-thr|/sd
#     """
#     # effective variance of noisy comparison
#     v = (w_eff * abs(val))**2 + (w_eff * abs(thr))**2
    
#     # safe standard deviation
#     sd = math.sqrt(v) if v > 0 else EPS
    
#     # comparison signal
#     z  = (thr - val) / max(sd, EPS)
#     p_left = _phi(z)

#     # d-prime (discriminability)
#     dprime = abs(val - thr) / max(sd, EPS)

#     # comparison time, protected by EPS
#     t_cmp  = T_COMP0 + T_COMP1 / max(dprime, EPS)

#     return p_left, t_cmp, {"sd": sd, "z": z, "dprime": dprime}
