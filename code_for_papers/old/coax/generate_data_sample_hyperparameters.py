...
df = pd.read_csv("./results/pop_em_subset/refit_all_assignments_detlocal-Jan-10.csv")
...
TOTAL_PARTICIPANTS_PER_REGIME = 25  # <-- change freely

rows_freq, out_path_freq = [], Path("results/CoAX simulated data (percent-matched).csv")
        ...
        for param_regime in ("w/ XAI", "w/o XAI"):
            ...
            for strat_name, n_part in alloc.items():
                ...
                for _ in range(n_part):
                    ...
                    if len(rows_freq) >= save_every_freq:
                        save_rows_to_csv(rows_freq, out_path_freq, write_header=first_write_freq)
