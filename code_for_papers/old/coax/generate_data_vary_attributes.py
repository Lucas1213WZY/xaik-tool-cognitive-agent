...
# 1) Paths + data
# -------------------------------
data_dir = os.path.join(parent_dir, "data", "datasets", "more attributes")
...
def should_skip_condition(xai_type: str, strat_name: str, param_regime: str) -> bool:
    ...
    if xai_type == "None" and param_regime == "w/ XAI":
        return True
    return False

# Training-size sweep: 1..15 step ~3
TRAIN_SIZES = [1, 3, 5, 7, 9, 11, 13, 15]  # edit freely (e.g., include 15 if you want)

# Total simulated participants per (dataset, xai_type, param_regime, n_train)
TOTAL_PARTICIPANTS_PER_REGIME = 200  # scale overall N here

# Output
out_path = Path("results/CoAX simulated data attributes-sweep.csv")
save_every = 20_000
...
def make_mask(df: pd.DataFrame, **wanted):
    ...
    for col, val in wanted.items():
        ...
        # print(f"make_mask: col={col}, val={val}, present={col in df.columns}, sum={mask.sum()}")
    return mask
    ...
    for lg in logs:
        ...
        rows_out.append({
            ...
            **config,
            ...
            "Num Attributes": int(num_attributes),
        })
        kept += 1
...
for ds_name, ds_cfg in DATASETS.items():
    ...
    if ds_name != "Wine Quality":
        print(f"  [SKIP] Dataset ➜ {ds_name} (only running 'Wine Quality' for now)")
        continue
    ...
    for xai_type in ("None", "Importance", "Attribution"):
        for num_attributes in range(1, 2):
            print(f"-- Varying num_attributes ➜ {num_attributes} --")
            ...
            ai_loader = (
                AIDatasetLoader(
                    ...
                    explanation_columns=[f"a{i}_i" for i in range(num_attributes)] if xai_type != "None" else [],
                )
                .filter_loader(
                    lambda dff,
                        app=ds_cfg["appId"] + f"_iter{num_attributes}",
                        mdl="mlp",
                        exp="shap":
            ...
            # print(AIDatasetLoader(
            #         feature_values_df=df_values,
            #         metadata_df=df_metadata,
            #         explanation_values_df=df_expl,
            #         explanation_columns=["a0_i", "a1_i", "a2_i", "a3_i", "a4_i"] if xai_type!="None" else [],
            #     ).explanation_values_df)

            # print(ai_loader.explanation_values_df)
