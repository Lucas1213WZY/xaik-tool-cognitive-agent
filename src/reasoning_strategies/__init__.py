...
def initialize_strategies():
    ...
    # Register CoXAM forward strategies
    StrategyRegistry.register(
        'lr_calculation',
        LRCalculation,
        LRCalculation(_cfg('lr_calculation', StrategyType.COXAM_FORWARD)).metadata
    )
    
    StrategyRegistry.register(
        'lr_heuristic',
        LRHeuristic,
        LRHeuristic(_cfg('lr_heuristic', StrategyType.COXAM_FORWARD)).metadata
    )
    
    StrategyRegistry.register(
        'dt_traversal',
        DTTraversal,
        DTTraversal(_cfg('dt_traversal', StrategyType.COXAM_FORWARD)).metadata
    )
