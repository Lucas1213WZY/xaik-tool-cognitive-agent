...
def tutorial_coxam_explanations():
    """
    Load CoXAM data and use the explainer registry to compute explanations.
    ...
    # Step 4a: Create a Decision Tree explainer
    # (Assumes you have a decision tree model saved as JSON)
    try:
        dt_explainer = registry.create(
            'decision_tree',
            model_json="path/to/decision_tree_model.json"
        )
        
        # Get instances to explain
        instances = loader.get_instances([0, 1, 2])
        
        # Generate explanations
        explanations = dt_explainer.apply(instances)
        print(f"✓ Decision Tree explanations: {explanations}")
        
    except FileNotFoundError:
        print("  (Note: Decision tree JSON not found in this example)")
    
    # Step 4b: Create a Logistic Regression explainer
    try:
        lr_explainer = registry.create(
            'logistic_regression',
            model_json="path/to/logistic_regression_model.json",
            feature_metadata=loader.get_metadata()
        )
        
        instances = loader.get_instances([0, 1, 2])
        explanations = lr_explainer.apply(instances)
        print(f"✓ Logistic Regression explanations: {explanations}")
        
    except FileNotFoundError:
        print("  (Note: Model JSON not found in this example)")
...
def tutorial_comparison():
    ...
    print(f"  • Available explainers: {registry.list_available()}")
    ...
    │ • Use: registry.create('explainer_type', ...).apply(instances)  │
...
def tutorial_mixed_access():
    """
    Load CoAX data but also register and use custom explainers.
    ...
    class CustomExplainer(BaseExplainer):
        ...
        def apply(self, instance):
            return {"custom_score": sum(instance.values()) / len(instance)}
        
        def apply_batch(self, instances):
            return [self.apply(inst) for inst in instances]
        
        def get_info(self):
            return {"name": "custom_explainer", "version": "0.1"}
    
    registry.register('custom', CustomExplainer)
    print(f"   Available explainers now: {registry.list_available()}")
    
    # Use the custom explainer
    print("\n3. Use the custom explainer alongside CSV explanations:")
    custom_exp = registry.create('custom')
    instances = [loader.get_instances([0])[0], loader.get_instances([1])[0]]
