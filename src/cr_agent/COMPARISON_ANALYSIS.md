...
As of the current refactoring, counterfactual strategies are now **imported from the reasoning_strategies layer** rather than implemented directly in cr_agent:
...
cr_agent/agents/counterfactual_strategies.py
    ...
    │
    └─ create_counterfactual_strategies() [factory]
         │
         └─ imports from src.reasoning_strategies.counterfactual:
            ├─ ZeroOutLRHeuristic
            ├─ ZeroOutLRDisplayed
            ├─ ChangeDTPath
            ├─ RecallChanges
...
- Instantiation of reasoning_strategies strategies
...
`create_counterfactual_strategies()` automatically:
1. Imports all 5 strategy classes from reasoning_strategies.counterfactual
...
✅ **Single source of truth** - strategies defined once in reasoning_strategies  
...
✅ **Consistent with existing API** - reasoning_strategies layer is the canonical implementation  
