...
Strategies are loaded from the reasoning_strategies API layer.
"""
...
# Import from reasoning_strategies API
...
def load_forward_strategies() -> Dict[str, Any]:
    """
    Load forward reasoning strategies from reasoning_strategies API layer.
    
    Returns:
        Dict mapping strategy names to strategy instances
    """
    # Create strategy configs
    dt_config = StrategyConfig(
        strategy_name="dt",
    ...
    lr_calc_config = StrategyConfig(
        strategy_name="lr_calc",
        strategy_type=StrategyType.COXAM_FORWARD,
        decay_param=0.5,
        extra_params={}
    )
    
    lr_heur_config = StrategyConfig(
        strategy_name="lr_heur",
        strategy_type=StrategyType.COXAM_FORWARD,
        decay_param=0.5,
        extra_params={}
    )
    ...
    strategies = {
        STRAT_DT: DTTraversal(dt_config),
        STRAT_LR_CALC: LRCalculation(lr_calc_config),
        STRAT_LR_HEUR: LRHeuristic(lr_heur_config),
    }
...
def run_meta_on_batch(
    *,
    ...
    for t in range(N):
        ...
        # Execute strategy step from reasoning_strategies layer
