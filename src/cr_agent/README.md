...
2. ⚠️  HEADLESS POLICIES vs COUNTERFACTUAL STRATEGIES
   ...
   - Counterfactual: Uses cognitive strategies from reasoning_strategies layer
     * Imported from: src.reasoning_strategies.counterfactual
...
│   └── counterfactual_strategies.py # Wrappers for reasoning_strategies.counterfactual
├── environments/
...
```python
from src.cr_agent.environments import CounterfactualMetaRouter
from src.cr_agent.agents import create_counterfactual_strategies

# Create router
routes = CounterfactualMetaRouter(
    meta_model=meta_ppo,
    strategies=create_counterfactual_strategies(),
    X_raw=X_raw,
    y_raw=y_raw,
...
```python
from src.cr_agent.environments import run_meta_on_batch
...
**CounterfactualStrategyWrapper** (from reasoning_strategies layer)
- Adapts reasoning_strategies.counterfactual strategies to cr_agent interface
- Wrapped strategies: ZeroOutLRHeuristic, ZeroOutLRDisplayed, ChangeDTPath, RecallChanges, MemoryBasedCF
- Methods: reset(), step()
- Factory function: create_counterfactual_strategies()
