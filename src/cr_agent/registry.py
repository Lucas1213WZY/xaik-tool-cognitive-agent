...
def _register_defaults() -> None:
    ...
    # Counterfactual Strategies (from reasoning_strategies layer)
    AgentRegistry.register(AgentMetadata(
        agent_type=AgentType.CF_STRATEGY_DT,
        name="ZeroOutLRHeuristic (reasoning_strategies)",
        description="Suggest zeroing high-magnitude features to flip LR decision - from reasoning_strategies.counterfactual",
    ))
    
    AgentRegistry.register(AgentMetadata(
        agent_type=AgentType.CF_STRATEGY_LR_HEUR,
        name="ZeroOutLRDisplayed (reasoning_strategies)",
        description="Use pre-computed LR for zero-out suggestions - from reasoning_strategies.counterfactual",
    ))
    
    AgentRegistry.register(AgentMetadata(
        agent_type=AgentType.CF_STRATEGY_LR_DISPLAYED,
        name="ChangeDTPath (reasoning_strategies)",
        description="Suggest feature changes to follow different DT path - from reasoning_strategies.counterfactual",
    ))
    
    AgentRegistry.register(AgentMetadata(
        agent_type=AgentType.CF_STRATEGY_RECALL_DT,
        name="RecallChanges (reasoning_strategies)",
        description="Retrieve stored counterfactual changes from memory - from reasoning_strategies.counterfactual",
    ))
    
    AgentRegistry.register(AgentMetadata(
        agent_type=AgentType.CF_STRATEGY_RECALL_LR,
        name="MemoryBasedCF (reasoning_strategies)",
        description="Use memory to synthesize alternative decisions - from reasoning_strategies.counterfactual",
    ))
