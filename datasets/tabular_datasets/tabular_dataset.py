...
class TabularDataset:
    ...
    def calculate_boundaries(self):
        ...
        for i in range(self.X.shape[1]):
            if i not in self.categorical_feature_indices:
                ...
                lower_bound = np.nanpercentile(self.X[:, i], 2)
                upper_bound = np.nanpercentile(self.X[:, i], 98)
    ...
    def use_specific_features(self, feature_names):
        # Determine indices of the specified features in the input order
        feature_indices = []
        for name in feature_names:
            if name not in self.feature_names:
                raise ValueError(f"Feature '{name}' not found in the dataset")
            feature_indices.append(self.feature_names.index(name))  # preserve order

        # Create new dataset with selected features (ordered)
        new_X = self.X[:, feature_indices]

        # Update feature names
        new_feature_names = feature_names  # already in desired order
        ...
        return self.copy_with_modifications(
            X=new_X,
            feature_names=new_feature_names,
            categorical_feature_options=new_categorical_feature_options,
            ordinal_feature_indices=new_ordinal_feature_indices,
            feature_boundaries=new_feature_boundaries
        )
    ...
    def prepare_instances_for_model(self, instances, one_hot_encode=True, normalize=True):
        if instances.ndim == 1:
            instances = instances.reshape(1, -1)

        encoded_X = np.copy(instances).astype(float)  # Copy to avoid modifying original instances

        # Normalize continuous features
        if self.feature_boundaries and normalize:
